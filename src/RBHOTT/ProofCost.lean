    ...
    /-- Count structural metrics on an expression. -/
    partial def countExpr (e : Expr) : ProofCost :=
      let rec go : Nat → Expr → ProofCost
        | d, Expr.app f a _ =>
            let cf := go (d+1) f
            let ca := go (d+1) a
            { (combine cf ca) with size := cf.size + ca.size + 1, apps := cf.apps + ca.apps + 1, maxDepth := Nat.max (d+1) (Nat.max cf.maxDepth ca.maxDepth) }
        | d, Expr.lam _ _ b _ =>
            let cb := go (d+1) b
            { cb with size := cb.size + 1, lambdas := cb.lambdas + 1, maxDepth := Nat.max (d+1) cb.maxDepth }
        | d, Expr.forallE _ _ b _ =>
            let cb := go (d+1) b
            { cb with size := cb.size + 1, lambdas := cb.lambdas + 1, maxDepth := Nat.max (d+1) cb.maxDepth }
        | d, Expr.letE _ v t b _ =>
            let cv := go (d+1) v
            let ct := go (d+1) t
            let cb := go (d+1) b
